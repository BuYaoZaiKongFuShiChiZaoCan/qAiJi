function audioMvmain() {
    ...
    if (findMusic(title.textContent)) {
        // 取列表编号
        let listNum = findMusic(title.textContent);
        // 创建一个video元素
        ...
        `;
        // 设置src属性为视频地址
        video.src = listNum.srcLocalhost;
        // 设置自动播放
        // video.autoplay = true;
        // 设置循环播放
        // video.loop = true;
        ...
        // 控制条
        video.controls = true;
        document.querySelector('#audioMv').appendChild(video);
        // 元素加载完成后
        ...
        title.textContent = listNum.id;
        let desc = document.createElement('p');
        desc.classList.add('audioMvDesc');
        desc.textContent = listNum.desc;
        document.querySelector('#audioMv').appendChild(title);
        document.querySelector('#audioMv').appendChild(desc);
    } else {
        tanChuang("mv不存在于文件列表中");
        // 移除#audioMv中的所有元素
        document.querySelector('#audioMv').textContent = null;
    }
}
