...
function headers() {
    if (display == 0) {
        ...
        if (hspanone) hspanone.style.transform = 'rotateZ(-45deg) translateX(-6px)';
        if (hspantwo) hspantwo.style.transform = 'rotateZ(45deg) translateX(-6px)';
...
if (qqh) {
    document.getElementById('profilePicture').src = 'http://q1.qlogo.cn/g?b=qq&nk=' + qqh + '&s=640';
}