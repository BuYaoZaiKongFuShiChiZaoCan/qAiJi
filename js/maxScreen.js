...
// 设置元素需要传入的，设置音乐列表
musicListHand.addEventListener('click', () => {
    ...
    document.querySelector('#dog #musicListHand').addEventListener('click', () => {/* musicListHand在dog里被复制了一次，又是屎山一部分了。。。 */
        scrollToIng2();
    })
    ...
    dog.onclick = function (e) { // 用element.onclick事件可以覆盖事件，不会注册两次onclick事件
        ...
        if (e.target.id !== '' && e.target.id !== 'musicListHand' && e.target.id !== 'dog') {
...
function scrollToIng2() {
    /* document.querySelector('#dog #musicListHand').addEventListener('click', () => {
...
let dogHeight
musicLrcHand.addEventListener('click', function () {
    dog.showModal();
    dogElementing = musicLrcHand;
    // 添加元素到弹框里
    dog.appendChild(createCloseBtn());
    dog.insertAdjacentHTML('beforeend', dogElementing.innerHTML);
    // 添加完后要将原id改掉
