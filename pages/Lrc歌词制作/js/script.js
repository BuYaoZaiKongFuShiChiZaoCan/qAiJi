...
window.tag.addEventListener('click', function (event) {
    event.stopPropagation();
    // 获取最后一个歌词元素
    let lastSpan = document.querySelector('#lrcList>li:last-of-type>span');
    if (audio.paused) {
        if (lastSpan.textContent != "00:00.0000") {
            tagNext();
            return;
        }
        audio.play();
...
function tagNext() {
    ...
    if (config.index) {
        ...
        window.lrcList.childNodes[index].childNodes[1].textContent = timeFormat(audio.currentTime);
        if (config.index2) {
...
window.Export.addEventListener('click', function (event) {
    ...
    for (let index = 0; index < window.lrcList.childNodes.length; index++) {
        ArrayLrc.push(window.lrcList.childNodes[index].childNodes[0].textContent) // 0是歌词，1是时间
        // 这里对时间处理事还应该加上小数点后几位的东西
        if (ExportConfig.decimalPlaces > 0) {
            let a = window.lrcList.childNodes[index].childNodes[1].textContent.split('.');
            ArrayTime.push(a[0] + '.' + a[1].substring(0, ExportConfig.decimalPlaces))
        } else {
            ArrayTime.push(window.lrcList.childNodes[index].childNodes[1].textContent.split('.')[0]);
        }
    }
